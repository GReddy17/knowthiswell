...
- **Quiz bank: 9+ questions per post, soft cap ~15, writer decides (founder rule, 2026-09-24).** Two parts, counted together: **(a) 2-3 inline `QuickCheck`s** placed in the article as reading checks (concept, examples, misconception), and **(b) the rest in `export const quiz: QuizBankItem[] = [...]`** at the top level of the post file (after `metadata`; import `QuizBankItem` from `@/types/post`). The bank is not shown while reading; it feeds the end-of-article "Test yourself" quiz (5 random per attempt, a fresh 5 on "Try again") and the KnowThisWell game. Rules for every question: plain string/boolean literals only (no JSX, variables or `<TermLink>`), or the extractor skips it; 3 options with exactly one `correct: true`; every option has an `explanation` saying *why* it's right or wrong; `difficulty` of `"easy"`, `"medium"` or `"hard"` (inline QuickChecks take an optional `difficulty` prop, default medium). Aim for about 3 easy, 4 medium, 2 hard across the whole set. Cover the key idea, each worked example, the misconception, the common mistakes and at least one apply-it scenario. No two questions testing the same fact, no trivia lifted from a single sentence, no "all of the above". Go past 15 only for genuinely broad topics.
