...
export default function Post() {
  return (
    <>
      ...
      <div className="prose-p">
      <strong>Reproductive system</strong> — produces the reproductive cells (sperm or eggs) and, in the female body, supports gestation; its organs are also a source of several of the body&apos;s sex hormones, tying it into the endocrine system&apos;s signaling network. <strong>Integumentary system</strong> — the skin, hair, and nails, and technically the body&apos;s largest organ by surface area, forms a physical barrier against pathogens and injury, helps regulate temperature (via sweat glands and blood vessel dilation, as in the fever example below), synthesizes vitamin D on sun exposure, and carries the touch, pressure, and temperature receptors that feed the nervous system — one input among the full set the body relies on for <TermLink href="/general-science-facts/the-five-senses">its five senses</TermLink>, with vision handled by its own specialized organ, <TermLink href="/general-science-facts/the-eye-and-how-vision-works">the eye</TermLink>. Between them, these 11 systems cover every major function the body performs — no system&apos;s job overlaps completely with another&apos;s, which is exactly why losing even one (kidney failure, for example) can&apos;t simply be compensated for by the others.
      </div>
